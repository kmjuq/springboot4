...
public class R2dbcInitializerConfig {
    ...
    public ConnectionFactoryInitializer initializer(ConnectionFactory connectionFactory) {
        ...
        schemaPopulator.addScript(new ClassPathResource("config/db/schema.sql")); // 位于 src/main/resources 下

        // 执行 data 脚本（插入测试数据）
        ResourceDatabasePopulator dataPopulator = new ResourceDatabasePopulator();
        dataPopulator.addScript(new ClassPathResource("config/db/data.sql"));
